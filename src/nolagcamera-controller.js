...
var NoLagCamera = function (options) {
  ...
  camera.close = function () {
    if (!_capturing)
      return;
    console.log('closing camera on', options.device);
    _capturing = false;
    console.log('sending SIGHUP to capture process');
    capture_process.kill('SIGHUP');
  };
  // Actual camera capture starting mjpg-stremer
  camera.capture = function (callback) {
    console.log('initiating camera on', options.device);
    ...
    fs.exists(options.device, function (exists) {
      ...
      // wooooo!  camera!
      console.log(options.device, ' found');
      _capturing = true;
      ...
      capture_process.on('close', function (code) {
